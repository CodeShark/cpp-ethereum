...
void Main::refresh()
{
	...
	if (m_client->changed())
	{
		...
		for (PeerInfo const& i: m_client->peers())
			ui->peers->addItem(QString("%3 ms - %1:%2 - %4").arg(i.host.c_str()).arg(i.port).arg(boost::chrono::duration_cast<boost::chrono::milliseconds>(i.lastPing).count()).arg(i.clientVersion.c_str()));
